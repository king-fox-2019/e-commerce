    ...
    <table class="table table-dark">
      ...
      <tbody v-for="(item,index) in mycart" :key="index">
        <tr v-if="item.status !== 'complete'">
          ...
          <td>{{ item.status }}</td>
          <td>
            <button type="button" class="btn btn-primary btn-sm" v-if="item.status == 'oncart'" @click="checkout(item._id, `process`, item.size[0]._id, item.quantities, item.size[0].stock), item.size[0].sold">BUY</button>
