...
class ProductController {
  ...
  static getDetailProduct (req, res, next) {
    Product.findById(req.query.productId)
      .then(function (product) {
        if (!product) throw { name: 'ProductNotFound', message: 'Product that you are looking is not found' }
  ...
  static updateProduct (req, res, next) {
    Product.findByIdAndUpdate(req.query.productId, req.body)
      .then(function (product) {
        if (!product) throw { name: 'ProductNotFound', messag: 'Product that you are looking is not found' }
      })
      .catch(next)
  }
}
